...
def fetch_html(url, css, user_agent="fxbg-event-bot/1.0", throttle=(2,5)):
    ...
    if "fredericksburgfreepress" in url:
        return fetch_freepress_calendar(url, default_tz=hints.get("timezone", "America/New_York"))
...
# --- ADD: Fredericksburg Free Press calendar scraper ---
...
def fetch_freepress_calendar(url: str, default_tz="America/New_York"):
    ...
    # Try several likely containers (WP/The Events Calendar / blocks)
    ...
    for node in candidates:
        ...
        # Date/Time text (many sites put datetime in <time> tags)
        ...
        if not start_txt:
            # look for readable text with month/day/time
